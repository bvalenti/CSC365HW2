    ...
    public static void main(String args[]) throws IOException, ClassNotFoundException {
        ...
        if (true) {
            ...
            for (int i = 0; i < urls.scrapedURLS.size(); i++) {
                b1.putInt(urls.scrapedURLS.get(i).length());
                toWrite = urls.scrapedURLS.get(i).getBytes();
                b1.put(toWrite);
            }
            b1.flip();
            ...
            for (int i = 0; i < urls.scrapedURLS.size(); i++) {
                BTree.insert(urls.scrapedURLS.get(i));
            }

            //Create and persist the frequency tables
            for (int i = 0; i < urls.scrapedURLS.size(); i++) {
                if (urls.scrapedURLS.get(i).endsWith("/")) {
                    url = urls.scrapedURLS.get(i).substring(0,urls.scrapedURLS.get(i).length()-1);
                    url = url.replace("//"," ").replace("/"," ").replace(":","").replace("<","").replace(">","").replace("*","").replace("?","");
                } else {
                    url = urls.scrapedURLS.get(i).replace("//"," ").replace("/"," ").replace(":","").replace("<","").replace(">","").replace("*","").replace("?","");
                }
                url = url + ".ser";
                ...
                tmp = parser.parseURL(urls.scrapedURLS.get(i));
